...
def main():
    ...
    if training_args.do_predict:
        ...
        ids = test_dataset['video-id']
        questions  = test_dataset['sent1']
        
        paragraghs = []
        for data in test_dataset:
          paragraghs.append([data["ending0"],data["ending1"],data["ending2"],data["ending3"]]) 

        contexts = []
        for p, pred in zip(paragraghs,predictions):
          contexts.append(p[pred])      

        data = [{'id':id, 'question':question, 'context':context} for id,question,context in zip(ids,questions,contexts)] 
